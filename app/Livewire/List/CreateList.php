...
class CreateList extends Component
{
    ...
    public function mount(Campaign $campaign){
        ...
        $this->rawData = $campaign->foreign_users()->with('foreing_aditional_info')->get()
                            ...
                            ->filter(fn ($item) => $item['department'])->values()->toArray();
    ...
    public function updated($property, $value){
        if ($property === 'department' && $value) {
            ...
            $this->municipalities = collect($this->rawData)->filter(fn ($item) => $item['department']['id'] == $value)
                                        ->pluck('municipality')->filter()->unique('id')->values()->toArray();
        }
        if($property === 'municipality' && $value){
            $this->neighborhoods    = [];
            $this->neighborhood     = null;
            $this->neighborhoods = collect($this->rawData)->filter(fn ($item) =>
                                        $item['municipality']
                                        && $item['municipality']['id'] == $value
                                        && $item['neighborhood']
                                    )->pluck('neighborhood')->unique()->values()->toArray();
        }
    }
