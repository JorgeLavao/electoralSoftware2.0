...
abstract class AbstractSupportersImport implements
    OnEachRow,
    WithHeadingRow,
    WithMultipleSheets,
    SkipsUnknownSheets,
    WithChunkReading
{
    ...
    protected array $counts = ['valid' => 0, 'warning' => 0, 'invalid' => 0];

    protected int $processedSinceFlush = 0;
    protected int $flushEvery = 2000;

    /**
     * Últimos errores (para vista previa).
     * Las subclases pueden llenarlo si quieren.
     */
    ...
    public function onUnknownSheet($sheetName)
    {
        ...
        // si hay hoja desconocida marcamos como "missingUsuariosSheet"
    ...
    public function onRow(Row $row)
    {
        ...
        // 3) Delega el tratamiento de la fila a la subclase
        $this->handleRowResult(
    ...
    /**
     * Cada subclase define qué hacer con la fila
     * (guardar en CSV, crear Supporter, etc.).
     */
    abstract protected function handleRowResult(
        int $rowIndex,
        array $data,
        string $status,
        array $messages
    ...
    protected function flushProgressToDb(): void
    {
    ...
    {
        // Por si al destruir la clase quedan filas sin volcar
        $this->flushProgressToDb();
    }

    // ------------------ LÓGICA COMPARTIDA ------------------ //

    protected function cleanRow(array $d): array
    {
        $fields = [
            'tipo_de_documento',
            'numero_de_documento',
    ...
    protected function validateRow(array $d): array
    {
        $errors = [];
        $warnings = [];
